/**
 * gulp-connect-includes = gulp-connect-ssi + esi support
 * <!--esi <esi:include src="/includes/metas.inc"/>-->
 * <!--#include virtual="/includes/metas.inc"-->
 */
...
var SSI = require('../node-ssi/index.js');
var parseurl = require('parseurl');
var async = require('async');
var extend = require('extend');
var iconv = require('iconv-lite');

// regex with esi support
const includeFileReg = /<!--(?:#include\s+|esi\s<esi:)(include|virtual)(?:\s*src)?=(['"])([^\r\n\s]+?)\2\s*(.*)-->/;
...
SSI.prototype.resolveIncludes = function (content, options, callback) {
  ...
  function insertInclude(next) {
    ...
    var tpath = null;
    if (isVirtual && options.dirname && RegExp.$3.charAt(0) !== '/') {
      tpath = path.join(options.dirname, RegExp.$3);
    }
    else {
      options.baseDir.some((baseDir) => {
        var _filePath = path.join(baseDir, RegExp.$3);
      ...
      if (!tpath) {
        tpath = RegExp.$3;
        console.log('ESI include not resolved:', tpath);
      }
    }
...
module.exports = function (opt) {
  ...
  return function (req, res, next) {
    ...
    var filePath = null;
    opt.baseDir.some((baseDir) => {
      var _filePath = path.join(baseDir, url);
      if (fs.existsSync(_filePath)) {
        filePath = _filePath;
        return true;
      }
